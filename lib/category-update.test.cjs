...
for (const valid of [true, false]) test(`edit submit prevents navigation and confirms returned category: ${valid}`, async () => {
  ...
  const code = ts.transpileModule('(' + extract('app/categories/page.js', 'handleUpdate') + ')', { compilerOptions: { jsx: ts.JsxEmit.React, target: ts.ScriptTarget.ES2020 }, fileName: 'handler.jsx' }).outputText;
