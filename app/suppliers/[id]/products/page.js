...
export default function SupplierProductsPage() {
  ...
  const fetchSupplierProducts = async (page = 1) => {
    try {
      ...
      const res = await fetch(
        `/api/v1/suppliers/${params.id}/products?page=${page}&limit=10`,
        {
          headers,
          cache: "no-store",
        },
      ...
      if (result.data?.[0]?.suppliers?.company_name) {
        setSupplier({
          name: result.data[0].suppliers.company_name,
          id: params.id,
        });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800 px-4 py-6">
            ...
            <>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {filteredProducts.map((product) => (
                      ...
                      >
                        <td className="px-4 py-2 text-slate-300 font-mono text-sm">
                          {product.id}
                        </td>
                        <td className="px-4 py-2 text-white font-medium text-sm">
                          {product.name}
                        </td>
              ...
              <div className="bg-slate-900/50 px-6 py-4 border-t border-slate-700 flex items-center justify-between flex-wrap gap-4">
                <p className="text-slate-400 text-sm">
                  Total: {pagination.total} produto
