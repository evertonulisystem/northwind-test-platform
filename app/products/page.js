...
export default function ProductsPage() {
  ...
  const fetchAllProducts = useCallback(async () => {
    try {
      ...
      if (token) {
        headers["Authorization"] = `Bearer ${token}`;
        console.log(
          "🐛 DEBUG PRODUCTS - Token encontrado, adicionando ao header",
        );
      } else {
      ...
      console.log(
        "🐛 DEBUG PRODUCTS - Produtos setados:",
        result.data?.length || 0,
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (selectedSupplier) {
      filtered = filtered.filter(
        (p) => p.suppliers?.company_name === selectedSupplier,
      );
    }
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative bg-slate-800/90 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-700 overflow-hidden">
            {loading ? (
              ...
              <div
                className="p-12 text-center text-slate-400"
                data-testid="no-products-container"
              >
                ...
                {(selectedCategory || selectedSupplier) && (
                  <p className="text-sm text-slate-500 mt-4">
                    Dica: Limpe os filtros para ver todos os produtos ou
                    adicione um novo com os filtros atuais
                  </p>
                )}
              </div>
            ) : (
              <>
                  ...
                  >
                    ...
                    <tbody>
                      {products.map((p) => (
                        ...
                        >
                          <td
                            className="px-4 py-2 text-slate-300 font-mono text-sm"
                            data-testid={`product-id-${p.id}`}
                          >
                            {p.id}
                          </td>
                          <td
                          ...
                          <td
                            className="px-4 py-2 text-green-400 font-semibold text-sm"
                            data-testid={`product-price-${p.id}`}
                          >
                            R$ {parseFloat(p.price || 0).toFixed(2)}
                          </td>
                          <td
                            className="px-4 py-2 text-slate-300 text-sm"
                          ...
                          <td
                            className="px-4 py-2 text-slate-300 text-sm"
                            data-testid={`product-supplier-${p.id}`}
                          >
                ...
                <div className="bg-slate-900/50 px-6 py-4 border-t border-slate-700 flex flex-wrap items-center justify-between gap-4">
                  ...
                  >
                    Mostrando {(pagination.page - 1) * ITEMS_PER_PAGE + 1}–
                    {Math.min(
      ...
      {showAddModal && (
        <AddProductModal
          ...
          preselectedSupplier={
            suppliers.find((sup) => sup.company_name === selectedSupplier)
              ?.id || ""
          }
        />
      )}
