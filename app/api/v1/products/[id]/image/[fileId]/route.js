...
export async function GET(request, { params }) {
  try {
    ...
    if (isVercel) {
      ...
      if (!targetFileName) {
        return NextResponse.json(normalizeApiBody({ data: null, mensagens: ['Imagem não encontrada para este ID.'] }), { status: 404 });
      }
      ...
      if (downloadError || !downloadData) {
        return NextResponse.json(normalizeApiBody({ data: null, mensagens: ['Erro ao baixar a imagem do storage.'] }), { status: 404 });
      }
    ...
    return NextResponse.json(normalizeApiBody({ data: null, mensagens: ['Erro interno ao buscar a imagem.', error.message || String(error)] }), { status: 500 });
  }
}
