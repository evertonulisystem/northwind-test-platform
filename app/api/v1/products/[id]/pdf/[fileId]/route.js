...
/**
 * @swagger
 * /api/v1/products/{id}/pdf/{fileId}:
 *   get:
 *     summary: Realiza o download de um PDF específico pelo seu ID
 *     tags: [Products]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: string
 *         description: ID do arquivo PDF
 *     responses:
 *       200:
 *         description: Arquivo PDF binário
 *       404:
...
export async function GET(request, { params }) {
  try {
    ...
    if (isVercel) {
      ...
      if (data) {
        const file = data.find(f => f.name.startsWith(`${fileId}_`) && f.name.endsWith('.pdf'));
      ...
      if (existsSync(productDir)) {
        ...
        targetFileName = files.find(f => f.startsWith(`${fileId}_`) && f.endsWith('.pdf'));
      }
