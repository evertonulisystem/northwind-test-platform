    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (result.mensagens?.[0]?.includes('usada por produtos')) {
          toast.error('⚠️ Esta categoria não pode ser excluída pois está sendo usada por produtos. Primeiro remova ou altere a categoria dos produtos.');
        } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-teal-800 to-green-900 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 text-center">
            <h1 className="text-5xl font-extrabold text-white mb-3">Gestão de Categorias</h1>
            <p className="text-xl text-emerald-200 mb-6">Cadastro e administração de categorias de produtos</p>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-md bg-slate-800 rounded-xl border border-slate-700 shadow-2xl">
            ...
            <form id="edit-category-form" onSubmit={handleUpdate} className="p-6 space-y-4">
              {/* Name */}
              <div>
                <label className="flex items-center gap-2 text-slate-300 text-sm font-medium mb-1">
                  <Tag className="w-4 h-4" />
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-sm bg-slate-800 rounded-xl border border-slate-700 shadow-2xl p-6">
            ...
            <p className="text-slate-300 mb-6">Tem certeza que deseja excluir esta categoria?</p>
